...
const inspectionForm = () => {
  ...
  useEffect(() => {
    const fetchSchools = async () => {
      try {
        setLoading(true);
        // Use lightweight dropdown endpoint to populate the select
        const response = await schoolServices.getSchoolDropdown();
    ...
    try {
      ...
      for (const [key, value] of Object.entries(formData)) {
        if (value instanceof FileList && value.length > 0) {
          ...
          const file = value[0];
          try {
            const res = await uploadServices.uploadSingleFile(file, 'inspections', null, access_token);
            if (res && res.success) {
              // Backend may return uploaded_file or various shapes — try common keys
              const uploaded = res.data?.uploaded_file || res.data?.file || res.data?.path || res.data?.data?.file || (typeof res.data === 'string' ? res.data : null) || null;
      ...
      // Submit inspection payload to backend
      ...
      if (response && response.success) {
        ...
        // backend may provide error message
        alert(response?.error || response?.data?.message || 'Failed to create inspection');
      }
    } catch (err) {
